...
all_test_smiles = []
all_test_vectors = []
data=pd.read_csv('/neims_test_smistdnpri_filtered_3500.csv')
test_SMILES=[]
test_mainSPEC=[]
test_mainpeakindex=[]
...
for i in range(len(data['inchikey'])):
    ...
    spectrummain = spec.Spectrum(mz=mzsmain,intensities=abundsmain,
                                metadata={'compound_name': 'substancemain'+str(i),'smiles':data['sdf_smiles'][i]})
    ...
    spectrumreplib = spec.Spectrum(mz=mzsreplib,intensities=abundsreplib,
                                metadata={'compound_name': 'substancereplib'+str(i),'smiles':data['sdf_smiles'][i]})
    test_replibspectrum.append(spectrumreplib)
spec.save_as_mgf(test_mainspectrum, '/testri3500_mainmeasured_spectrum.mgf')
spec.save_as_mgf(test_replibspectrum, '/testri3500_replibmeasured_spectrum.mgf')
